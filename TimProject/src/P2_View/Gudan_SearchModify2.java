...
public class Gudan_SearchModify2 extends JDialog implements FocusListener {
			...
			public void actionPerformed(ActionEvent arg0) {
				if (table.getSelectedRow() > -1) {
					int result = JOptionPane.showConfirmDialog(null,
							"팀 \"" + list.get(table.getSelectedRow()).getGroup_Name() + "\"으로 선수 이적신청을 완료하시겠습니까?",
							"이적신청", JOptionPane.OK_CANCEL_OPTION);
					if (result == 0) {
						new Player_TransferDAO().transferJoin(new Player_TransferDTO(0, PLAYER_CODE, now_Group_Code, list.get(table.getSelectedRow()).getGroup_Code(), 0));
